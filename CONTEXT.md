...
**Note etag** — the token a client echoes back as If-Match, carried by every stat-backed note response: `Note`, `UpdateNoteResponse`, `MoveNoteResponse`, `NoteDetail`, and the MCP note tools (`notes.ETag`, RFC3339Nano). It exists because there are two clocks: those responses `stat` the file, while every index-backed `updated_at` — `/tree`, note lists, tag lists, backlinks — is whole seconds out of SQLite and can therefore never match. Naming the valid token separately is what keeps a client from reaching for the nearest `updated_at`. `NoteSummary` deliberately has none — every endpoint returning one, `GET /notes/by-path` included, reads whole seconds out of SQLite.

The move response is the subtle one. A rename is an `os.Rename` and leaves mtime alone, but the wiki-link rewrite that follows a title change can touch the renamed note itself, because a note linking to its own old title is its own referrer. `PostNoteMove` therefore re-reads after the rewrite rather than returning the mtime `Move` saw.
